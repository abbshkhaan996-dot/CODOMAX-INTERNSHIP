...
async function renderFeaturedHero() {
  ...
  if (!el) return;
  let posts;
  try { posts = await fetchPosts(); } catch (err) { el.style.display = "none"; return; }
...
async function renderHeroStats() {
  ...
  if (!el) return;
  let posts;
  try { posts = await fetchPosts(); } catch (err) { el.textContent = "Couldn't reach the server."; return; }
...
async function renderCategoryShelf() {
  ...
  let posts;
  try { posts = await fetchPosts(); } catch (err) { posts = []; }
...
async function renderHomeLedger(filterCategory, searchTerm) {
  ...
  if (!list) return;
  let allPosts;
  try { allPosts = await fetchPosts(); } catch (err) {
    list.innerHTML = `<div class="empty-state"><h3>Can't reach the server</h3><p>Make sure the backend is running, then refresh.</p></div>`;
    return;
  }
  ...
  // remember the current category/search values so either control can trigger
  // a re-render without wiping out whatever the other control had selected
  ...
  if (filterSelect && !filterSelect.dataset.wired) {
    ...
    filterSelect.addEventListener("change", () => renderHomeLedger(filterSelect.value, undefined));
  }
  ...
  if (searchInput && !searchInput.dataset.wired) {
    ...
    searchInput.addEventListener("input", () => {
      ...
      debounceTimer = setTimeout(() => renderHomeLedger(undefined, searchInput.value), 200);
    });
  }
  ...
  if (posts.length === 0) {
    const reason = term
      ? `Nothing matches "${escapeHtml(activeSearch.trim())}"${activeCategory && activeCategory !== "all" ? ` in ${escapeHtml(activeCategory)}` : ""}`
      : `Nothing under "${escapeHtml(activeCategory)}" yet`;
    list.innerHTML = `<div class="empty-state"><h3>${reason}</h3><p>Try a different search or category.</p></div>`;
    return;
  }
...
async function renderPostDetail() {
  ...
  let post = null;
  if (id) {
    try { post = await fetchPost(id); } catch (err) { post = null; }
  }
...
async function initCreateBlogForm() {
  ...
  if (editId) {
    try {
      const candidate = await fetchPost(editId);
      if (candidate && candidate.authorId === user.id) editingPost = candidate;
    } catch (err) { editingPost = null; }
  ...
  form.addEventListener("submit", async (e) => {
    ...
    try {
      if (editingPost) {
        await updatePost(editingPost.id, { title, tag, image: coverImageData, content });
        msg.textContent = "Changes saved — heading to your dashboard.";
      } else {
        await createPost({ title, tag, image: coverImageData, content });
        msg.textContent = "It's live — heading to your dashboard.";
      }
      msg.className = "form-msg success show";
      setTimeout(() => { window.location.href = "dashboard.html"; }, 500);
    } catch (err) {
      showFormError(msg, err.message || "Couldn't publish that — please try again.");
...
async function initProfileForm() {
  ...
  form.addEventListener("submit", async (e) => {
    ...
    if (name.length < 2) return showFormError(msg, "Your name needs to be at least 2 characters.");

    const wantsPasswordChange = currentPassword || newPassword || confirmNewPassword;
    if (wantsPasswordChange) {
      if (!currentPassword) return showFormError(msg, "Enter your current password to set a new one.");
      if (newPassword.length < 6) return showFormError(msg, "New password must be at least 6 characters.");
      if (newPassword !== confirmNewPassword) return showFormError(msg, "New passwords don't match.");
    }
    ...
    try {
      ...
      msg.className = "form-msg success show";
      initNav("profile"); // refresh the nav in case the name changed
    } catch (err) {
      showFormError(msg, err.message || "Couldn't update your profile — please try again.");
    } finally {
      submitBtn.disabled = false;
    }
  });
